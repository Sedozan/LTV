"""
Validates that the expense-ratio lookups in the public results
file match the raw Finance file.
"""
...
from specialty_ltv.jobs.expense.process_expense_data.process_raw_expense import (
    read_raw_expenses,
)
...
CLAIMS_CONSTANT = 0.047832  # source: finance excel, tab LTV_SPL, cell GG13
# or claims_renew_er in process_raw_expense.py

CHANNEL_SEGMENTS = {"agt": ["EA", "IA"], "ccc": ["ccc"], "web": ["web"]}

TOL_REL = 1e-3  # tolerance of how far the ratios can differ
RATIO_COLS = [
    f"expense_ratio_{b}_{t}"
...
def validate_expense_ratios(spark) -> None:
    ...
    assert (
        len(fin_df) == 1
    ), f"expected 1 CW row from read_raw_expenses, got {len(fin_df)}"
    ...
    for channel in CHANNEL_SEGMENTS:
        ...
        if len(sample) == 0:
            logger.warning(
                f"expense_ratio_validate: no {channel} policy in public file, skipping"
            )
            continue
        row = sample.iloc[0]
        ...
        for bucket, components in BUCKET_COMPONENTS.items():
            for term in ("new", "renew"):
                ...
                if ok:
                    logger.info(
                        f"expense_ratio_validate pass {channel} {bucket}_{term}: "
                        f"{actual:.6f}"
                    )
                else:
                    failures.append(
