...
Source B = book-level $ from the State Hybrid Expense Ratio file, row 15, recomputed
           per channel by hand.  A must equal B.

Never uses the *_exp dollar columns (those are premium * ratio -> circular).

PUBLIC SCHEMA NOTES (differ from the internal e006scl_* names):
  - prefix `e006scl_` is dropped        -> expense_ratio_acquisition_new, ...
  ...
  - premium_to_float / premium_to_surplus are dropped from public (capital ratios)
"""
import pytest
...
# ---------------------------------------------------------------------------
# Finance file, LTV_SPL row 15, NEW file (SPL v1.3.0 / Classic v9.2.0).
# (segment) -> (new, renew).  Renter uses WEB; classic agt = EA + IA.
# THIS BLOCK IS THE ONE PLACE TO UPDATE ON A FINANCE REFRESH.
# earned_prem assumed unchanged by the MVR reclass -- confirm against the sheet.
# ---------------------------------------------------------------------------
FIN = {
 "earned_prem":       {"EA": (446836970, 2325310435), "IA": (8130333, 160350749), "WEB": (57401898, 41073401)},
 "other_acq_x_mkt":   {"EA": (144407671, 3818976),    "IA": (105409, 2788),       "WEB": (3037449, 291655)},
 "other_overhead":    {"EA": (34599354, 143942307),   "IA": (600272, 2497287),    "WEB": (8515334, 5943023)},
 "commissions":       {"EA": (33163547, 172580936),   "IA": (527984, 10413180),   "WEB": (0, 0)},
 "state_premium_tax": {"EA": (9990303, 51988886),     "IA": (159052, 3136903),    "WEB": (1496442, 1070765)},
 "write_offs":        {"EA": (1294960, 6738887),      "IA": (20617, 406611),      "WEB": (193971, 138794)},
 "assessments":       {"EA": (279904, 1456603),       "IA": (4456, 87888),        "WEB": (41927, 30000)},
 "pay_fees":          {"EA": (-7480277, -38926873),   "IA": (-119091, -2348768),  "WEB": (-1120467, -801740)},
 "misc_income_exp":   {"EA": (-589889, -3069744),     "IA": (-9391, -185222),     "WEB": (-88359, -63225)},
 "other_lifetime":    {"EA": (15114122, 50716166),    "IA": (315518, 1058736),    "WEB": (3420887, 2216578)},
 "lower_funnel_advt": {"EA": (22578399, 1963339),     "IA": (684496, 59521),      "WEB": (33791771, 2938415)},
 "other_mkt":         {"EA": (17221608, 14885359),    "IA": (415619, 359237),     "WEB": (4764191, 1225317)},
}

# public bucket name -> Finance components summed into the numerator
PUBLIC_BUCKET_COMPONENTS = {
    "acquisition": ["other_acq_x_mkt"],
    "overhead":    ["other_overhead"],
    "commission":  ["commissions"],                       # public: singular
    "service":     ["state_premium_tax", "write_offs", "assessments",   # public name for lifetime
                    "pay_fees", "misc_income_exp", "other_lifetime"],
    "marketing":   ["lower_funnel_advt", "other_mkt"],
}
# claims excluded: hardcoded constant, no Finance lookup (guarded separately below)

CHANNEL_SEGMENTS = {"agt": ["EA", "IA"], "web": ["WEB"], "ccc": ["CCC"]}

# (id, repo_label, s3_path, policy_id, expected_channel)
CASES = [
    ("classic", "classic-specialty-ltv",
     "tmx-smsiweb/classic-specialty-ltv/prod/release/NB_Classic_SPL_v9.2.0/score/public_results/",
     "1844586908208", "agt"),
    ("renter", "specialty-ltv",
...
def spark():
    ...
    from specialty_ltv import paths as p          # buckets are the same Scality bucket
...
def test_claims_is_constant(policy_row, term):
    # claims is a hardcoded constant, not a Finance lookup. Guard the value so a
    # silent change is caught. CONFIRM 0.047832 against the current pipeline constant.
    _, row = policy_row
    assert row[f"expense_ratio_claims_{term}"] == pytest.approx(0.047832, abs=1e-6)
